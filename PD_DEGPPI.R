...
DEG_list <- readLines("~/Documents/PhD/Parkinsons/Parkinsons_Code/Results/GeneExpression/FoldChange/Sp_LRRK2_FiltALSPDblood_ALLgenes.txt")
# 
...
# setwd("/Users/clairegreen/Documents/PhD/Parkinsons/Parkinsons_Code/Results/Park_PPI")
...
setwd("/Users/clairegreen/Documents/PhD/Parkinsons/Parkinsons_Code/Results/PD_PPI/")

mart_table <- read.csv("HGNC2Uniprot.csv", header = T) #A table with the uniprot codes for the DEGs
uniprot_gene <- mart_table$Uniprot
...
DEG_PPI <- read.csv("DEG_PPITable.csv")

DEG_PPI <- subset(DEG_PPI, DEG_PPI$Gene1 !="-")
DEG_PPI <- subset(DEG_PPI, DEG_PPI$Gene2 !="-")
 
write.csv(DEG_PPI, "PD_DEG_PPI.csv", row.names = F, quote = F)
